...
tb <- round(ftable(svyby(~tabaco$educa, ~status, design =fumo, FUN = svymean, keep.var = TRUE))*100,1)
#Chi-square test
...
#with SE
round(ftable(svyby( ~Q092, ~status, design =fumo, FUN = svymean, keep.var = TRUE))*100,1)
...
#with SE
round(ftable(svyby( ~Q132, ~status, design =fumo, FUN = svymean, keep.var = TRUE))*100,1)
